...
namespace ServiceIMSS
{
    public class OrdenesTrabajo
    {
        ...
        public Result ActualizaCategorizacion(Categorizacion data)
        {
            ...
            try
            {
                var client = new RestClient(UrCliente + "&webService=IMSS_WS_GestionDeOrdenesIMSS_PM");
                var request = new RestRequest("", Method.Post);
                request.AddHeader("Content-Type", "text/xml charset=utf-8");
                request.AddHeader("SOAPAction", "urn:IMSS_WS_GestionDeOrdenesIMSS_PM/Actualiza_Categorizacion_IMSS");
                var body = @"<?xml version=""1.0"" encoding=""utf-8""?>" +
                @"<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:urn=""urn:IMSS_WS_GestionDeOrdenesIMSS_PM"">" +
                @"  <soap:Header>" +
                ...
                @"         <urn:TipoDeOperacion>A_CAT_WO</urn:TipoDeOperacion>" +
        ...
        public Result ActualizaOrdenes(OrdenTrabajo data)
        {
            Result result = new Result();
            ...
            try
            {
                ...
                var request = new RestRequest("", Method.Post);
                request.AddHeader("Content-Type", "text/xml charset=utf-8");
                request.AddHeader("SOAPAction", "urn:IMSS_WS_GestionDeOrdenesIMSS_PM/Actualiza_Ordenes_IMSS");
                var body = @"<?xml version=""1.0"" encoding=""utf-8""?>" +
                @"<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:urn=""urn:IMSS_WS_GestionDeOrdenesIMSS_PM"">" +
                @"  <soap:Header>" +
                @"      <urn:AuthenticationInfo>" +
                ...
                @"   </soap:Header>" +
                @"   <soap:Body>" +
                @"      <urn:Actualiza_Ordenes_IMSS>" +
                @"         <urn:TipoDeOperacion>A_WO</urn:TipoDeOperacion>" +
                @"         <urn:NombreProveedor>" + NombreProveedor + "</urn:NombreProveedor>" +
                @"         <urn:IDTicketIMSS>" + data.IDTicketRemedy + "</urn:IDTicketIMSS>" +
                @"         <urn:IDTicketProveedor>" + data.IDTicketInvgate + "</urn:IDTicketProveedor>" +
        ...
        public Result ActualizaPrioridad(PrioridadOT data)
        {
            Result result = new Result();
            ...
            try
            {
                ...
                var request = new RestRequest("", Method.Post);
                request.AddHeader("Content-Type", "text/xml charset=utf-8");
                request.AddHeader("SOAPAction", "urn:IMSS_WS_GestionDeOrdenesIMSS_PM/Actualiza_Prioridad_IMSS");
                var body = @"<?xml version=""1.0"" encoding=""utf-8""?>" +
                @"<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:urn=""urn:IMSS_WS_GestionDeOrdenesIMSS_PM"">" +
                @"  <soap:Header>" +
                @"      <urn:AuthenticationInfo>" +
                ...
                @"         <urn:IDTicketIMSS>" + data.IDTicketRemedy + "</urn:IDTicketIMSS>" +
                @"         <urn:IDTicketProveedor>" + data.IDTicketInvgate + "</urn:IDTicketProveedor>" +
                @"         <urn:Prioridad>" + data.Prioridad + "</urn:Prioridad>" +
                @"         <urn:FechaActualizacion>" + DateTime.Now.ToString() + "</urn:FechaActualizacion>" +
                @"      </urn:Actualiza_Prioridad_IMSS>" +
                @"   </soap:Body>" +
                @"</soap:Envelope>" +
        ...
        public Result AdicionaNotas(Comentario data)
        {
            ...
            try
            {
                ...
                var request = new RestRequest("", Method.Post);
                request.AddHeader("Content-Type", "text/xml charset=utf-8");
                request.AddHeader("SOAPAction", "urn:IMSS_WS_GestionDeOrdenesIMSS_PM/Adiciona_Notas_Ordenes_IMSS");
                var body = @"<?xml version=""1.0"" encoding=""utf-8""?>" +
                @"<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:urn=""urn:IMSS_WS_GestionDeOrdenesIMSS_PM"">" +
                @"  <soap:Header>" +
                @"      <urn:AuthenticationInfo>" +
